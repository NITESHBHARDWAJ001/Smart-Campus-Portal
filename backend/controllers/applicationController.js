...
exports.createApplication = async (req, res) => {
  try {
    ...
    if (!placement) {
      return res.status(404).json({ success: false, message: 'Placement opportunity not found' });
    }

    if (!placement.active) {
      return res.status(400).json({ success: false, message: 'This placement opportunity is no longer accepting applications' });
    }
    ...
    if (existingApplication) {
      return res.status(400).json({ success: false, message: 'You have already applied for this placement opportunity' });
    }
...
exports.deleteApplication = async (req, res) => {
  try {
    ...
    if (req.user.role === 'student' && populatedApplication.studentId.toString() !== req.user.id) {
    ...
    if (req.user.role === 'student') {
      const placement = await Placement.findById(populatedApplication.placementId);
      if (placement && new Date(placement.deadline) < new Date()) {
        return res.status(400).json({ success: false, message: 'Cannot withdraw application after deadline has passed' });
      }
    }

    await populatedApplication.deleteOne();
