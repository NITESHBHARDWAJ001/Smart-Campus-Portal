  ...
  const handleAddPlacement = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        await getPlacements(); // Refresh placements after adding
      } else {
        setAlertMessage({
          type: 'danger',
  ...
  const handleEditPlacement = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setTimeout(() => setAlertMessage({ type: '', message: '' }), 3000);
        await getPlacements(); // Always refresh placements after edit
      } else {
        setAlertMessage({
          type: 'danger',
  ...
  const handleDeletePlacement = async () => {
    try {
      ...
      if (result.success) {
        ...
        setTimeout(() => setAlertMessage({ type: '', message: '' }), 3000);
        await getPlacements(); // Always refresh placements after delete
      } else {
        setAlertMessage({
          type: 'danger',
  ...
  const handleToggleActive = async (placement) => {
    try {
      ...
      if (result.success) {
        ...
        setTimeout(() => setAlertMessage({ type: '', message: '' }), 3000);
        await getPlacements(); // Always refresh placements after toggle
      } else {
        setAlertMessage({
          type: 'danger',
  ...
  const handleUpdateApplicationStatus = async (applicationId, status) => {
    try {
      ...
      if (result.success) {
        ...
        await getPlacements(); // Also refresh placements in case status affects placement stats
      } else {
        setAlertMessage({
          type: 'danger',
  ...
  const handleDeleteApplication = async (applicationId) => {
    try {
      ...
      if (result.success) {
        ...
        await getPlacements(); // Also refresh placements in case stats change
      } else {
        setAlertMessage({
          type: 'danger',
    ...
    <div className="placement-container">
                      ...
                      <td>
                        <ButtonGroup size="sm">
                          ...
                          <Button 
                            ...
                            title="Edit"
                            disabled={false} // Always allow edit
                          >
                            <FaEdit />
                          </Button>
                          <Button 
      ...
      <Modal show={showDeleteModal} onHide={handleCloseModals}>
        ...
        <Modal.Footer>
          ...
          <Button variant="danger" onClick={handleDeletePlacement} disabled={loading}>
            Delete Placement
          </Button>
        </Modal.Footer>
      </Modal>
                ...
                <tbody>
                  {applications.map(application => (
                    <tr key={application._id}>
                      ...
                      <td>{application.studentId?.email}</td>
                      <td>{application.studentId?.rollNumber}</td>
                      <td>{application.studentId?.department}</td>
                      <td>
                        <Badge bg={application.status === 'Selected' ? 'success' : application.status === 'Rejected' ? 'danger' : 'warning'}>
                          {application.status}
                        </Badge>
                      </td>
                      <td>
                        <ButtonGroup>
                          <Button size="sm" variant="success" onClick={() => handleUpdateApplicationStatus(application._id, 'Selected')} disabled={loading || application.status === 'Selected'}>
                            <FaUserCheck /> Accept
                          </Button>
                          <Button size="sm" variant="danger" onClick={() => handleUpdateApplicationStatus(application._id, 'Rejected')} disabled={loading || application.status === 'Rejected'}>
                            <FaUserTimes /> Reject
                          </Button>
                          <Button size="sm" variant="outline-danger" onClick={() => handleDeleteApplication(application._id)} disabled={loading}>
                            <FaTrash /> Delete
                          </Button>
                        </ButtonGroup>
                      </td>
