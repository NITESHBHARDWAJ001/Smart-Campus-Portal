...
const StudentPlacements = () => {
  ...
  const handleApply = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        await getMyApplications(); // Refresh applications so My Applications tab updates
      } else {
        setAlertMessage({
          type: 'danger',
  ...
  const myApplications = applications.filter(app => {
    ...
    if (typeof app.studentId === 'string') return app.studentId === user._id;
    return app.studentId._id === user._id;
  });
          ...
          {loading ? (
            ...
            <Row>
              {filteredPlacements.map(placement => {
                const hasApplied = myApplications.some(app => app.placementId._id === placement._id);
                return (
                    ...
                    <Card className="placement-card h-100">
                      <Card.Body>
                        ...
                        <div className="d-flex justify-content-between align-items-center">
                          ...
                          {!hasApplied && !isDeadlinePassed(placement.deadline) && (
                            <Button
                              variant="primary"
                              size="sm"
